...
export async function resolveChallenge(env: Env, challengeId: string): Promise<ResolverResult> {
  ...
  const rows = (await sql`
    SELECT claim, resolution_criteria as "resolutionCriteria"
    FROM challenges
    WHERE id = ${challengeId}
    LIMIT 1
  `) as unknown as Array<{ claim: string; resolutionCriteria: string }>;
  ...
  await sql`
    INSERT INTO resolution_runs (id, challenge_id, exa_query, source_urls, ai_rationale, proposed_outcome, confidence)
    VALUES (${newId("res")}, ${challengeId}, ${exaQuery}, ${JSON.stringify(result.sourceUrls)}, ${result.shortRationale}, ${result.outcome}, ${result.confidence})
  `;

  if ((result.outcome === "YES" || result.outcome === "NO") && result.confidence >= 0.85) {
    await sql`
      UPDATE challenges
      SET status = 'provisional_resolved',
          provisional_outcome = ${result.outcome},
          dispute_deadline_at = ${provisionalDisputeDeadline()},
          updated_at = now()
      WHERE id = ${challengeId}
        AND status = 'open'
    `;
